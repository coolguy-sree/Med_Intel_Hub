...
st.set_page_config(page_title="AI-Driven Medical Intelligence Hub", page_icon="🩺", layout='wide')

# Sidebar content
st.sidebar.markdown("<h1 style='text-align: center; color: #007bff;'>🩺 Medical Intelligence Hub</h1>", unsafe_allow_html=True)
st.sidebar.image("utils/ph3.png", use_container_width=True)
st.sidebar.markdown("""
    <div style='padding: 10px; font-size: 16px; line-height: 1.5; color: #333;'>
        The Disease Prediction & Medical Recommendation system uses AI to analyze symptoms, predict diseases, assess health risks, and suggest personalized treatments—helping you make informed healthcare decisions.
    </div>
...
st.title("🩺 AI-Driven Medical Intelligence Hub")

# Disease Prediction Section
st.markdown("### 📝 Disease Prediction Based on Symptoms")

user_input = st.text_area("Enter symptoms (comma-separated):", placeholder="e.g., headache, constipation, nausea", key="symptom_input", help="Enter symptoms separated by commas")
...
if st.button("Predict Disease"):
    if user_input:
        ...
        if patient_symptoms:
            ...
            st.markdown(f"**Precautions:** {', '.join(str(item) for item in precautions if item)}")
            st.markdown(f"**Medications:** {', '.join(str(item) for item in medications if item)}")
            st.markdown(f"**Recommended Diet:** {', '.join(str(item) for item in rec_diet if item)}")
            st.markdown(f"**Recommended Workout:** {', '.join(str(item) for item in workout if item)}")
        else:
            st.error("Invalid symptoms detected. Please check and try again.")
    else:
        st.warning("Please enter at least one symptom.")

# Disease Description Search Section
st.markdown("### 🔍 Search for Disease Description")

disease_query = st.text_input("Type a disease name to get recommendations:", placeholder="Start typing...", key="disease_query", help="Start typing a disease name")
