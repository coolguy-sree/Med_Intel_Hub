...
st.sidebar.markdown(
    "<h2 style='color: #000000;'>📌  Description</h2>", unsafe_allow_html=True
)
st.sidebar.image("utils/ph4.png", use_container_width=True)
st.sidebar.markdown(
    "<p class='sidebar-text' style='color: #000000;'>Our AI-powered Drug Recommendation System uses NLP and cosine similarity to analyze medicines and recommend the most relevant alternatives, ensuring accurate, data-driven, and personalized treatment options.</p>",
...
with col1:
    selected_medicine_name = st.selectbox(
        "Select a medicine:", sorted(medicines["Drug_Name"].values)
    )
with col2:
...
desc = description_data.loc[
    description_data["Drug_Name"] == selected_medicine_name, "Description"
]
if not desc.empty:
    st.markdown(
        f"""
        <p style='background-color: #ffffff; color: #000000; padding: 12px; border-radius: 10px; font-size: 16px;'>
